"""
Stage 2, Tier 1 + Tier 2 + Tier 3 eval -- "call vs clarify" binary classification.

Extends score_call_vs_clarify_tier2.py (unchanged, still re-runnable as its
own six-model record) with the Tier 3 online on-policy GKD checkpoint
(gkd_student_tier3, trained via train_gkd.py -- lmbda=0.5, beta=0.5,
truncated JSD loss, unfiltered 2760-example split, 1 epoch, ~1h57m).

Same metric, same held-out set, same reasoning as before: this only asks
whether the model correctly decides TO call a function or not, sidestepping
every exact_args formatting-convention artifact. Ground truth is
unambiguous (convo[1]["from"] == "function_call").

Usage:
    python score_call_vs_clarify_tier3.py
"""
import json
import re
...
MODELS = {
    ...
    "tier2_unfiltered": {"path": "./tier2_student_unfiltered", "four_bit": False},
    "tier2_filtered":   {"path": "./tier2_student_filtered",   "four_bit": False},
...
def main():
    ...
    print("\ntier3_gkd = online on-policy GKD (student generates, teacher scores via truncated")
    print("generalized JSD; lmbda=0.5 mixes on/off-policy). The comparison that matters: does")
    print("on-policy correction beat tier2_unfiltered's already-strong offline-logit result?")
