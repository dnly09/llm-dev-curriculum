"""
Stage 2, Tier 1 + Tier 2 eval -- head-to-head scoring on the untouched 240-example held-out set.

Extends score_students.py (Tier 1-only, unchanged, still re-runnable as its own
record) with the two Tier 2 offline-logit-KD checkpoints:
    ...
    3. tier1_distilled    -- SFT on filtered teacher completions (Tier 1 payoff)
    4. tier2_unfiltered   -- offline logit KD, full 2760-example train split
    5. tier2_filtered     -- offline logit KD, 2157-example filtered train split
    6. teacher             -- Qwen2.5-7B-Instruct, 4-bit           (ceiling)
...
Usage:
    python score_students_tier2.py
"""
import json
import re
...
def main():
    ...
    print("=" * 70)
    print("\nSuccess criterion: tier2 variants should beat baseline_sft on exact_args --")
    print("unlike tier1 (sequence-level KD), logit-level KD has a real mechanistic channel")
    print("(the teacher's full distribution) that plain SFT on hard labels cannot access.")
